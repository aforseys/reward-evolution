...
class RewardMachineEnv(gymnasium.Wrapper):
    def __init__(self, env, rm_files):
        ...
        flatdim = gymnasium.spaces.flatdim(self.observation_dict)
        self.observation_space = dict()
        for obs in env.observation_space: 
            s_low  = float(env.observation_space[obs].low[0])
            s_high = float(env.observation_space[obs].high[0])
            self.observation_space[obs] = spaces.Box(low=s_low, high=s_high, shape=(flatdim,), dtype=np.float32)
    ...
    def reset(self):
        ...
        return self.get_observation(self.obs, self.current_rm_id, self.current_u_id, False), self.info
...
class RewardMachineWrapper(gymnasium.Wrapper):
    ...
    def _get_rm_experience(self, rm_id, rm, u_id, obs, action, next_obs, env_done, true_props, info):
        ...
        next_u_id, rm_rew, rm_done = rm.step(u_id, true_props, info, env_done)
        done = rm_done or env_done
        rm_next_obs = self.env.get_observation(next_obs, rm_id, next_u_id, done)
        return (rm_obs,action,rm_rew,rm_next_obs,done), next_u_id

    def _get_crm_experience(self, obs, action, next_obs, env_done, true_props, info): #QUESTION: why is next obs passed? 
        """
